...
class Lldp_interfaces(ConfigBase):
    ...
    def set_state(self, want, have):
        ...
        lldp_intf_ele = junosutils.build_subtree(root, 'lldp')
    ...
    def _state_merged(self, want, have):
        ...
        lldp_intf_xml = []
        for config in want:
            lldp_intf_root = junosutils.build_root_xml_node('interface')

            if config.get('name'):
                junosutils.build_child_xml_node(lldp_intf_root, 'name', config['name'])

            if config.get('disable') is not None:
                if config['disable']:
                    junosutils.build_child_xml_node(lldp_intf_root, 'disable')
                else:
                    junosutils.build_child_xml_node(lldp_intf_root, 'disable', None, {'delete': 'delete'})
