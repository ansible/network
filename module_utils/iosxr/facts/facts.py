...
from ansible.module_utils.iosxr.facts.l3_interfaces.l3_interfaces import L3_interfacesFacts
...
class Facts(FactsArgs, FactsBase):
    ...
    def get_facts(self, module, connection, gather_subset=['!config'], gather_network_resources=['all']):
        ...
        if valid_network_resources_subsets:
            resources_runable_subsets = self.generate_runable_subsets(module, gather_network_resources,
                                                                      valid_network_resources_subsets)
    ...
    def _get_l3_interfaces(module, connection):
        return L3_interfacesFacts(L3_InterfacesArgs.argument_spec, 'config', 'options').populate_facts(module,
                                                                                                       connection)
