...
class L3_interfaces(ConfigBase):
    ...
    def _get_common_xml_node(self, name):
        root_node = junosutils.build_root_xml_node('interface')
        junosutils.build_child_xml_node(root_node, 'name', name)
        intf_unit_node = junosutils.build_child_xml_node(root_node, 'unit')
    ...
    def _state_merged(self, want, have):
        ...
        for config in want:
            ...
            junosutils.build_child_xml_node(unit_node, 'name',
                                            str(config['unit']))
            if config.get('ipv4'):
    ...
    def build_ipaddr_et(self, config, unit_node, protocol='ipv4',
                        delete=False):
        family = junosutils.build_child_xml_node(unit_node, 'family')
        inet = 'inet'
        if protocol == 'ipv6':
            inet = 'inet6'
        ip_protocol = junosutils.build_child_xml_node(family, inet)
        for ip_addr in config[protocol]:
            if ip_addr['address'] == 'dhcp' and protocol == 'ipv4':
                junosutils.build_child_xml_node(ip_protocol, 'dhcp')
            else:
                ip_addresses = junosutils.build_child_xml_node(
                    ip_protocol, 'address')
                junosutils.build_child_xml_node(
        ...
        for config in want:
            if config['name'] not in existing_l3_intfs:
                ...
                junosutils.build_child_xml_node(unit_node, 'name',
                                                str(config['unit']))
                family = junosutils.build_child_xml_node(unit_node, 'family')
                ipv4 = junosutils.build_child_xml_node(family, 'inet')
                intf = next(
                    (intf for intf in have if intf['name'] == config['name']),
                    None)
                if 'ipv4' in intf:
                    if 'dhcp' in [x['address'] for x in intf.get(
                        'ipv4') if intf.get('ipv4') is not None]:
                        junosutils.build_child_xml_node(
                            ipv4, 'dhcp', None, {'delete': 'delete'})
                    else:
                        junosutils.build_child_xml_node(
                            ipv4, 'address', None, {'delete': 'delete'})
                ipv6 = junosutils.build_child_xml_node(family, 'inet6')
                junosutils.build_child_xml_node(ipv6, 'address', None,
                                                {'delete': 'delete'})
            intf_xml.append(root_node)
        return intf_xml
