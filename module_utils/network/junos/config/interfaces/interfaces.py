...
class Interfaces(ConfigBase):
    ...
    def _state_merged(self, want, have):
        ...
        for config in want:
            intf = junosutils.build_root_xml_node('interface')
            junosutils.build_child_xml_node(intf, 'name', config['name'])
            ...
            for field in intf_fields:
                if config.get(field):
                    junosutils.build_child_xml_node(intf, field, config[field])

            if config.get('duplex'):
                junosutils.build_child_xml_node(intf, 'link-mode', config['duplex'])

            if config.get('enable') is False:
                junosutils.build_child_xml_node(intf, 'disable')

            holdtime = config.get('hold_time')
            if holdtime:
                holdtime_ele = junosutils.build_child_xml_node(intf, 'hold-time')

                for holdtime_field in ['up', 'down']:
                    junosutils.build_child_xml_node(holdtime_ele, holdtime_field, holdtime.get(holdtime_field, ''))
    ...
    def _state_deleted(self, want, have):
        ...
        for config in want:
            ...
            for field in intf_fields:
                junosutils.build_child_xml_node(intf, field, None, {'delete': 'delete'})

            if not config['name'].startswith('lo'):
                junosutils.build_child_xml_node(intf, 'link-mode', None, {'delete': 'delete'})

            junosutils.build_child_xml_node(intf, 'disable', None, {'delete': 'delete'})

            holdtime_ele = junosutils.build_child_xml_node(intf, 'hold-time')
            for holdtime_field in ['up', 'down']:
                junosutils.build_child_xml_node(holdtime_ele, holdtime_field, None, {'delete': 'delete'})
            intf_xml.append(intf)
