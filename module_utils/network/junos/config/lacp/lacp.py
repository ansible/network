...
class Lacp(ConfigBase):
    ...
    def set_state(self, want, have):
        ...
        :returns: the list xml configuration necessary to migrate the current configuration
                  to the desired configuration
        """
        root = junosutils.build_root_xml_node('chassis')
        ethernet_ele = junosutils.build_subtree(root, 'aggregated-devices/ethernet')
    ...
    def _state_merged(self, want, have):
        ...
        for config in want:
            lacp_root = junosutils.build_root_xml_node('lacp')
            junosutils.build_child_xml_node(lacp_root, 'system-priority', config.get('system_priority'))
            if config.get('link_protection') == 'non-revertive':
                junosutils.build_subtree(lacp_root, 'link-protection/non-revertive')
            elif config.get('link_protection') == 'revertive':
                junosutils.build_child_xml_node(lacp_root, 'link-protection')
    ...
    def _state_deleted(self, want, have):
        ...
        lacp_root = junosutils.build_root_xml_node('lacp')
        junosutils.build_child_xml_node(lacp_root, 'system-priority', None, {'delete': 'delete'})
        element = junosutils.build_child_xml_node(lacp_root, 'link-protection', None, {'delete': 'delete'})
        junosutils.build_child_xml_node(element, 'non-revertive', None, {'delete': 'delete'})
